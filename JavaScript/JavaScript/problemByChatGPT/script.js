...
console.log(
    "Longest Alternating Seqeuce:",
    longestAlternatingSequence("101010")
);
console.log(
    "Longest Alternating Seqeuce:",
    longestAlternatingSequence("110011")
);
console.log(
    "Longest Alternating Seqeuce:",
    longestAlternatingSequence("000111")
);
...
console.log(
    "Longest Parallel Sequence:",
    longestParallelSequence("11000111101010111")
);
...
console.log(
    "Unique Character:",
    uniqueChar(["apple", "banana", "abc", "police"])
);
...
console.log(
    "Object Key Lenght Sorted By Value:",
    objectKeyLengthSortedByValue({ a: "hi", b: "hello", c: "hey" })
);
...
console.log(
    "Second Largest Num:",
    secondLargestNumAnotherWay([10, 20, 4, 2, 99])
);
    ...
    const checkIfNestedArr = (subArr) => {
        for (let i = 0; i < subArr.length; i++) {
            if (Array.isArray(subArr[i])) {
                //今見てる要素がArrayだったらcheckIfNestedArrを呼び出して深い階層の処理をさせる
...
console.log(
    "Count Unique Distinct Element:",
    countUniquElmnt([1, 2, 2, 3, 4, 4, 5])
);

// Wirte a function that takes array of integers and retruns new array that moves all 0s in an array to the end, while keeping the relative order of none-zero elements.
const moveZeros = (arr) => {
    let newArr = [];
    let zeroCount = 0;
    for (let i = 0; i < arr.length; i++) {
